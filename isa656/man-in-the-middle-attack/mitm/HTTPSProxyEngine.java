...
public class HTTPSProxyEngine extends ProxyEngine
{
	...
	final byte[] buffer = new byte[40960];
		...
		if (httpsConnectMatcher.find()) {//then we have a proxy CONNECT message!
		    ...
		    // Get java certChain originated from server
		    iaik.x509.X509Certificate serverCertificate = new iaik.x509.X509Certificate(serverCertChain[0].getEncoded());
		    // Convert java certChain to iaik cert for convenience, only one actually
		    ...
		    System.out.println("Copy certificate from remote server that client visited.");
		    localProxy = m_proxySSLEngine.createServerSocket(serverDN, serverSerialNumber, serverCertificate);
		    }catch(Exception e){
		    }
...
//		    System.out.println(serverCN);
		   ...
		   // localProxy.getLocalPort()
		    ...
		    try{
		    	localPort = localProxy.getLocalPort();
		    }catch(Exception e){
		    }
		    
		    final Socket sslProxySocket =
			getSocketFactory().createClientSocket(getConnectionDetails().getLocalHost(),localPort);			  
	...
	public final ServerSocket createServerSocket(Principal remoteServerDN, BigInteger serialNumber, iaik.x509.X509Certificate serverCertificate) throws IOException, java.security.GeneralSecurityException, Exception {
	    MITMSSLSocketFactory ssf = new MITMSSLSocketFactory(remoteServerDN, serialNumber, serverCertificate);
