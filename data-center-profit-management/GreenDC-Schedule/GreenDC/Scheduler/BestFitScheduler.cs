...
namespace GreenSlot.Scheduler
{
    public class BestFitScheduler : SchedulerBase, IScheduler
    {
        ...
        public BestFitScheduler(Config config, ProblemSetting ps)
            : base(config, ps)
        {
        ...
        public override void ScheduleJob(Job job)
        {
            ...
            if (CurrentTime + job.RequiredTimeSlots >= Config.TimeSlots) return;
            //find best schedule
        ...
        {            
            ...
            for (var t = CurrentTime; t <= job.SlackTime; ++t )
            {
                ...
                {
                    var assignedJobs = Cluster.GetJobsByNodeLists(nodeList, t, job.RequiredTimeSlots);
                    ...
                    for (var tp = 0; tp < job.RequiredTimeSlots; ++tp)
                    {
        ...
        public override bool AssignPreemptiveJob(Job job)
        {
            ...
            if (job.AssignedTimeSlots == job.RequiredTimeSlots) job.Scheduled = true;
        ...
        {           
            ...
            if (job.AssignedTimeSlotsList.Count > job.RequiredTimeSlots)
            {
                Console.WriteLine("AssignTimeSlots={0}, RequiredTimeSlots={1}", job.AssignedTimeSlotsList.Count, job.RequiredTimeSlots);
                throw new Exception();
            }
            ...
            if (nodeList.Count == job.RequiredNodes)
            {
                for (int ot = t; ot <= job.SlackTime && job.ScheduledCost > 0; ot++)
                {
                    ...
                    for (int st = ot; st < job.Deadline; st++)
                    {
                        ...
                        if (assignTimeSlots.Count == job.RequiredTimeSlots)
                        {
                            var usedGreen = 0;
                            ...
                            if (usedGreen >=  job.RequiredNodes * job.RequiredTimeSlots)
                            {
                                return true;
                            }
