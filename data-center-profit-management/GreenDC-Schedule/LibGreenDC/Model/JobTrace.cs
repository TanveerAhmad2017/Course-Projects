...
//        //            if (job.ArrivalTime + job.ProcessingTime >= config.TimeSlots - 1)
//        //            {
//        //                continue;
//        //            }

//        //            //if (job.ProcessingTime > config.SlotPerDay /4 && config.IsFilterLargeJob)
//        //            //{
...
//        //            if (job.ProcessingTime <= 0) continue;
...
//                    //if (job.ArrivalTime + config.AvgProcessTime >= config.TimeSlots - 1) continue;
//                    //if (job.ArrivalTime + config.LeadTime + job.ProcessingTime >= config.TimeSlots - 1) continue;
//                    //if ((job.ArrivalTime) > config.TimeSlots) break;
...
//                j.ProcessingTime = 5 * j.ProcessingTime;
//                j.ProcessingTime = Math.Min(j.ProcessingTime, config.SlotPerDay / 2);
//                j.RequiredNodes = (int)Math.Min((double)j.RequiredNodes, config.ClusterNodeNum / 2);

//                rntJob.Add(j);
//            });

//            rntJob.RemoveAll(jb => { return (jb.ArrivalTime + jb.ProcessingTime >= config.TimeSlots); });
//            //filter out large jobs
//            rntJob.RemoveAll(jb => { return (jb.ProcessingTime >= config.TimeSlots / 4); });

//            for (int j = 0; j < rntJob.Count; j++)
//            {
//                Job jb = rntJob[j];

//                var deadline = RandomGenerator.GetRandomInteger(jb.ArrivalTime + jb.ProcessingTime, config.TimeSlots);

//                if (config.isDeadlineBounded)
//                {
//                    deadline = jb.ArrivalTime + (int)(jb.ProcessingTime / config.L);
//                }
...
//                var energyCost = jb.ProcessingTime * jb.RequiredNodes * config.RevenueRate;
