function avgWorkload()
    %Build Figure
    setting = '48-16-10';
    
   workload = [ 0.09 0.30 1 0.658984375  0.7513 1.104]
...
%     errorbar(x,bf(1,:),bfErr(1,:),'Color', 'g','LineWidth', 2)
    ...
    upper = max( workload(1,:))

     upper = ceil(upper/1)*1;        
   ...
   set(axes1,'XTick',[0.2 0.6 0.8 1.0 1.2 1.4],'XTickLabel',[0.2 0.6 0.8 1.0 1.2 1.4],'XGrid','on','YGrid','on');
    ...
    set(get(axes1,'YLabel'),'String', 'Workload','FontSize',30,'FontWeight','bold');
    ...
    filename = strcat('.\figures\workload_',setting)
    saveas(gcf, filename, 'pdf') %Save figure  
    saveas(gca, strcat(filename, '.eps'),'psc2') %Save figure 
    
     filename = strcat('C:\Users\hwang14\Dropbox\Meerkats\greenSlot\greenSlot-CS773\Figures\workload_', setting)
    saveas(gcf, filename, 'pdf') %Save figure  
    saveas(gca, strcat(filename, '.eps'),'psc2') %Save figure 
